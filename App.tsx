      ...
      <main className="flex-1 flex flex-col min-w-0 h-full bg-[#05060b] overflow-x-hidden">
        <header className="pt-[env(safe-area-inset-top)] border-b border-slate-800 bg-[#0a0b10]/90 z-40 shrink-0">
          <div className="h-14 px-4 flex items-center overflow-x-auto no-scrollbar space-x-2">
            ...
            {['chat', 'graph', 'notes', 'customise'].map(t => (
              <button key={t} onClick={() => setActiveTab(t as any)} className={`px-4 py-3 text-[10px] mono uppercase font-bold shrink-0 border-b-2 transition-all ${activeTab === t ? 'border-cyan-500 text-cyan-400' : 'border-transparent text-slate-600 hover:text-slate-400'}`}>
                {t}
              </button>
            ))}
          </div>
